    ...
    var body: some View {
        ...
        .task(id: wantsPlayback) {
            ...
            do {
                ...
                try Task.checkCancellation()
                try await LocalPlaybackSession.shared.activate(movie: true)
                try Task.checkCancellation()
                guard wantsPlayback, scenePhase == .active else { return }
        ...
        .onChange(of: scenePhase) { _, phase in
            if phase != .active {
                player?.pause()
                // Cancel an in-flight activation as well as already playing media.
                wantsPlayback = false
            }
        }
        .onDisappear {
