...
final class SourceEvidenceUITests: XCTestCase {
    ...
    @MainActor func testEntryFromThreadsPreservesThreadFinder() throws {
        ...
        // The system overflow menu preserves the label, not the SwiftUI link's identifier.
        let entry = app.buttons["Search saved evidence"]
        XCTAssertTrue(entry.waitForExistence(timeout: 5)); entry.tap()
        XCTAssertTrue(app.searchFields["Search saved text"].waitForExistence(timeout: 5))
        search("receipt", in: app)
        XCTAssertTrue(result("NOVA-42", in: app).waitForExistence(timeout: 10))
        capture("Saved evidence entered from Threads", app)
        // Active native search shows Close; dismiss it before testing navigation Back.
        let closeSearch = app.navigationBars.buttons["Close"]
