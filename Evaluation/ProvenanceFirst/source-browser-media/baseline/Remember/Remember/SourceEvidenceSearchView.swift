        ...
        List {
            Section {
                ...
                Text(model.scope == .current
                     ? "Search current, unarchived source text. Results are related passages, not verified answers."
                     : "Includes archived memories and earlier revisions. Older passages may no longer reflect the current state.")
                ...
                Section {
                    ContentUnavailableView(
                        ...
                        description: Text(model.didSearch
                            ? "Try another word or Include history. No match does not prove that a fact was never recorded; some media may not have searchable text."
...
private struct SourceEvidenceResultRow: View {
    let hit: SourceEvidenceHit
...
private struct SourceEvidenceDetailView: View {
    let hit: SourceEvidenceHit
    let sequence: Int64
