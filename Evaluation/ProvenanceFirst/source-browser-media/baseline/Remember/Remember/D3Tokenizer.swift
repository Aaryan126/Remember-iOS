...
nonisolated struct D3Tokens: Codable, Sendable, Equatable {
    let input_ids: [[Int32]]
    let attention_mask: [[Int32]]
    let token_type_ids: [[Int32]]
}

nonisolated enum D3Error: Error { case invalidVocabulary, unsupportedLength, missingResource(String), invalidOutput, invalidFixture }
...
nonisolated struct D3Tokenizer: Sendable {
    ...
    init(vocabularyURL: URL) throws {
        ...
        guard words.count == 30_522, Set(words).count == words.count else { throw D3Error.invalidVocabulary }
        ...
        guard vocabulary["[PAD]"] == 0, vocabulary["[UNK]"] == 100,
              ...
              vocabulary["[MASK]"] == 103 else { throw D3Error.invalidVocabulary }
    }

    func encode(first: String, second: String, length: Int) throws -> D3Tokens {
        guard [256, 512].contains(length) else { throw D3Error.unsupportedLength }
        ...
        if second.isEmpty {
            ...
            let used = a.count + 2
            return D3Tokens(input_ids: [[101] + a + [102] + Array(repeating: 0, count: length - used)],
        ...
        return D3Tokens(input_ids: [ids], attention_mask: [mask], token_type_ids: [types])
    }
