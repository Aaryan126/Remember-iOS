...
## Current implementation — unified search, 24 September 2026

Opening follow-up: user confirmed the closing fix but found the grid leading
the search bar on focus. A single-scroll-container implementation removes that
measured mismatch while retaining the snapshot fade and restoring browsing
position. **11 UI and 6 lifecycle tests pass**, with per-frame opening/closing
checks at the top and scrolled positions. Signed build, in-place installation and
physical iPhone smoke test all passed. Verified pre/post copies retain all 8
memories, 8 originals and 171 history events unchanged. The app is open for user
review. See [opening follow-up](../Evaluation/ProvenanceFirst/search-handoff/OPENING-FOLLOWUP.md).
The paragraphs below describe the preceding installed implementation.

Latest search-dismissal candidate combines a short in-memory frozen-results fade
with a stable `.geometryGroup()` around the browsing library. Rendered-coordinate
traces identified the remaining artifact as an extra approximately 100-point
movement caused by scroll-container reparenting, not ordinary keyboard motion.
The corrected top/dark and scrolled/light traces show zero extra displacement
across eight populated clears/closes and six empty controls. Current-source
**11/11 UI and 6/6 handoff tests pass**, and intermediate-frame review confirms
the fade instead of a hard content cut. Rejected bitmap and scroll-edge trials
were removed. Approved A+B cleanup recovered 423.78 MiB while protecting backups.
The updated build is installed in place and opens normally. Two phone automation
attempts timed out before app assertions, so manual review of perceived smoothness
is still required. Verified pre/post-launch copies preserve all 8 originals,
8 memory rows and 171 history events unchanged, with SQLite integrity intact.
The earlier phone smoke pass below did not establish smoothness. See the
[new handoff investigation](../Evaluation/ProvenanceFirst/search-handoff/REPORT.md).

Earlier follow-up: frame tracing isolated a second layout change in the outgoing
results layer during the installed fade. A small source fix removes that fade
and scopes unanimated removal to the results branch, leaving native search and
keyboard motion intact. Following the second approved cleanup, **11/11 UI tests
and an extra focused top-of-library trace test passed**. Empty/matching/no-match
closes share the same native scroll-offset spring; recordings were reviewed.
The third approved cleanup recovered 654.23 MiB net and unblocked deployment.
The signed candidate is installed in place after fresh verified backups. The
final physical-phone smoke passed on the unchanged build; earlier failures at
cancellation/initial focus remain recorded, with orientation changes observed.
Their cause is not proven by the passing rerun. Post-install comparison confirms
healthy SQLite and all 8 memory rows, 171 events and 8 originals unchanged.
Perceived smoothness still needs user confirmation. The reserve is
unchanged and no further cleanup is authorized.
See the
[follow-up status](../Evaluation/ProvenanceFirst/search-overlay/REPORT.md).

Earlier installed follow-up (fade now superseded): header gaps are halved from 20 to 10 points, keeping
the centered controls and 44-point filter target. Closing populated search now
uses a short, Reduce Motion-aware opacity handoff to the preserved library;
empty-field focus/dismissal remains native. Ten simulator UI tests, recorded
fictional motion-frame review and the physical-phone smoke passed (no phone
retry needed). Pre/post snapshots preserve all 8 memory rows, 171 events and 8
originals unchanged. The user's confirmation of perceived smoothness is pending.
See the [dismissal deployment report](../Evaluation/ProvenanceFirst/search-dismissal-deployment/REPORT.md).

Earlier spacing checkpoint:

Search-header spacing is now installed: the title/count/filter control are
center-aligned, with equal space above and below the default results header.
Ten simulator UI tests and the physical-phone spacing/menu/motion smoke passed
(phone automation startup required one retry). Verified snapshots preserve all
8 memory rows, 171 events and 8 originals unchanged. The user has confirmed the
preceding native-focus motion fix feels smooth. See the
[spacing deployment report](../Evaluation/ProvenanceFirst/search-spacing-deployment/REPORT.md).

Native empty-search behavior is now installed: focusing an empty field leaves the
browsing grid visible, and results/options appear after typing. The custom fade
from the earlier transition fix is removed; iOS owns search/keyboard motion.
Verification passed: 18 native tests, 9 simulator UI cases and one physical-phone
empty-focus/repeated-dismissal/menu smoke. All 8 memory rows, 171 events and 8
originals remain unchanged. User confirmation of perceived smoothness was
subsequently received on 24 September.
See the [native-search deployment report](../Evaluation/ProvenanceFirst/search-motion-deployment/REPORT.md).

UX follow-up installed: the two filters are now in a compact **Search options**
menu, with an active-filter status/Reset only when needed. Repeated help and the
explicit AI-search prompt moved into the menu too. This quieter presentation is
now installed in the main app on the physical iPhone. The signed build and menu
smoke passed (1/1 after an automation-initialization timeout on the first attempt).
All 8 memory rows, 171 history events and 8 original files remain unchanged.
See [search behavior](unified-memory-search.md) and the
[compact-menu deployment report](../Evaluation/ProvenanceFirst/search-options-deployment/REPORT.md).

Memories now hosts ordinary search plus **Include history** and **Source text only**
options. Matching saved passages open exact retained versions, not the latest
replacement. The duplicate Threads menu entry is removed. Keyboard submission is
local; AI search remains explicit and Ask AI's answer path is unchanged.
See the [plan, validation and limitations](unified-memory-search.md).
This update is now installed in the main Remember app on the physical iPhone.
The signed app/extension build and normal-app navigation smoke passed (1/1, no
skips or reported runtime warnings). Verified pre/post snapshots preserve all
8 memory rows, 171 history events and 8 originals unchanged. See the
[unified-search deployment report](../Evaluation/ProvenanceFirst/unified-search-deployment/REPORT.md).

## Previous phone deployment — 20 September 2026

**Source-first browsing is now installed in the user's main Remember app**, after
the separately approved controlled development update. Open Threads → More (…) →
Search saved evidence. The signed normal app/share extension build and one navigation
smoke passed on the physical iPhone, with zero skipped tests or reported runtime
warnings. Verified pre/post snapshots retain all 8 memory rows, 171 history events
and 8 original files unchanged. No uninstall, forced regrouping, cloud request or
preference change. See the [main-app deployment report](../Evaluation/ProvenanceFirst/main-app-deployment/REPORT.md).

Earlier verification used an isolated phone build. The foundation, Threads entry, saved-version detail UI and
media/navigation/accessibility follow-up are complete at their review boundaries.
Final follow-up: 29 native tests, 10 simulator UI cases and 10 physical iPhone UI
cases passed, zero skips/reported runtime warnings; full production-entrypoint
build-only and signed diagnostic builds passed. Twelve final phone captures were
visually reviewed. This is engineering verification, not improved model accuracy.

At that earlier checkpoint the normal Remember installation was not replaced;
Evidence Check used fictional data and separate storage, without private-vault access
or model/cloud calls. The later main-app update above includes a local private backup
and data-preservation checks. D3 remains
the existing experimental local organizer. Failed relationship/answer-verifier
candidates remain excluded; Pro organization remains research, not enabled.

Next recommendation: try the feature in normal Remember, then use feedback to guide
bounded accessibility/audio-route/large-library hardening. The earlier
[fixture walkthrough](../Evaluation/ProvenanceFirst/source-browser-media/USER-REVIEW.md)
remains available in Evidence Check but is not a prerequisite for personal use.
No automatic new model sweep. This development installation is not a public-release
qualification or a new retrieval/model-accuracy result.
The chronological research history below preserves prior gates and failures;
earlier “not integrated” statements refer to their named experiments/checkpoints.
...
The next approved checkpoint is a [media/navigation/accessibility follow-up](../Evaluation/ProvenanceFirst/source-browser-media/PLAN.md).
It checks original-file previews, inline playback, current-River round trips and
maximum-text usability with fictional fixtures. Checkpoint 2's 119 bound inputs
were archived and verified before edits. See [follow-up status](../Evaluation/ProvenanceFirst/source-browser-media/STATUS.md)
for actual results; an in-progress run is not a pass. This work does not change D3,
introduce cloud requests, or authorize deployment over the user's real Remember app.
The 20 September continuation passed all nine unchanged UI cases on the physical
iPhone, with visible image/text originals and successful Quick Look/River returns.
The earlier simulator suite's two preview failures remain recorded. Device review
also exposed a largest-text clipping issue and reproduced synchronous audio-session
activation warnings. Intermediate fixes passed 29 native tests. Waiting for the
actual image canvas also passed the simulator image-return test; all seven new
media cases passed in the latest run before the existing 26 GiB resource guard
interrupted it. The full ten-case suite and updated phone/build checks remain
incomplete at that resource stop; see the
[follow-up report](../Evaluation/ProvenanceFirst/source-browser-media/REPORT.md).
The user then explicitly approved a scoped 28 GiB cap with the original baseline
and 10 GiB reserve retained. Final saved sources passed **29 native tests**, **10
simulator UI cases** (zero skips/runtime warnings), the production-entrypoint
build-only check and a signed isolated phone build. Final simulator captures show
the original image, correct preview return, wrapped largest-text scope, voice
completion/replay state and visible video. After a saved device-unlock hold and an
evening resource interruption, the unchanged phone retry **1789916047531086000**
passed all ten UI cases with zero skips/reported runtime warnings. Twelve exported
phone captures were inspected, including visible originals, correct preview return,
current River, largest text, older revisions and unavailable/corrupt media states.
Only ~644 MiB of regenerable compiler caches from this isolated media build were
removed; signed products/results/source/captures and the original accounting baseline
were preserved. This checkpoint is complete and stopped for review, not a full
production-readiness or matching-quality qualification. Real Remember is unchanged.
