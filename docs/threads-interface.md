...
- Find a thread is a native search field on the Threads root, matching the Memories search style. Focus alone keeps the map visible behind the keyboard. Inline suggestions appear only for a non-whitespace query and filter immediately as it changes. Closing search returns to the map without another navigation level. The map remains mounted while search results cover it.
- Inline search supports local name search, case/diacritic-insensitive matching of all entered words, full Dynamic Type titles and memory counts. Search never calls a hosted model. Empty or whitespace-only queries show the map; no-match and empty-library states are explicit.
...
- `ProjectView` owns the single thread destination for both map and search. The map supplies a selection callback and shares the root’s transition namespace. Each opening has a fresh presentation identity to isolate nested Memory/activity state; repeated taps cannot replace an active destination. Map selections retain the node zoom transition unless Reduce Motion is enabled; search uses standard navigation.
...
Map navigation follow-up: removed the separate UUID destination registration inside `ProjectGraphView` and centralized map/search presentation in `ProjectView`, using a dedicated route type and fresh view identity for each opening. The initial three-cycle simulator probe passed before the change, so the exact intermittent failure was not reproduced. The expanded `testRepeatedMapOpenAfterMemoryReadOnly` covers ordinary reopen, interactive back from Memory, rapid double-taps, search → Memory → map, and the expected thread heading on every map opening. It passed on simulator and iPhone. `xcodebuild test` also passed `testExistingMapFocusHoldDragAndReturn` and `ThreadNavigationUITests/testExistingThreadsFinderAndHistoryReadOnly` on simulator (`/tmp/remember-map-navigation-fix.xcresult`). The iPhone repeated-open result is in `/tmp/remember-map-navigation-fix-phone.xcresult`; a separate search check in that run failed to type because the search field lacked keyboard focus. An unchanged `xcodebuild test-without-building` retry of the iPhone search/history check passed (`/tmp/remember-map-navigation-search-phone-retry.xcresult`). A simulator recording was inspected frame by frame to verify the node zoom transition. `git --no-optional-locks diff --check` passed and the updated app was installed on iPhone. Changed files for this follow-up: `ProjectView.swift`, `ProjectGraphView.swift`, `RememberUITests.swift`, and this document.
