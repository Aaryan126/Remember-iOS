...
private struct SearchTransitionFixtureRoot: View {
    ...
    var body: some View {
        Group {
            if ready {
                if ProcessInfo.processInfo.arguments.contains("--memory-return") {
                    TabView {
                        ...
                        ProjectView(model: project, onAsk: {})
                            .tabItem { Label("Threads", systemImage: "point.3.connected.trianglepath.dotted") }
                        Text("Fixture settings")
                            .tabItem { Label("Settings", systemImage: "gearshape") }
                    }
                } else if ProcessInfo.processInfo.arguments.contains("--search-status-loading") {
                    SearchStatusFixture()
        ...
        .task {
            ...
            do {
                ...
                if ProcessInfo.processInfo.arguments.contains("--memory-return") && !filenames.contains("return-photo.png") {
                    ...
                    try image.pngData()!.write(to: files.url(for: "return-photo.png"))
                    let date = Date(timeIntervalSince1970: 1_800_000_001)
                    let photo = MemoryItem(
                        id: UUID(uuidString: "11111111-1111-1111-1111-111111111111")!, kind: .image,
                        createdAt: date, importedAt: date, updatedAt: date, state: .indexed,
                        originalFilename: "return-photo.png", userCaption: "A fictional teal photo",
                        title: "Return transition photo", summary: "Synthetic image for navigation checks",
