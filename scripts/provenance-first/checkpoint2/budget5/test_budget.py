import unittest
from approved_run import accounting, GIB


class BudgetTests(unittest.TestCase):
    def test_approved_range(self):
        self.assertEqual(accounting(37*GIB, 42*GIB, GIB, GIB)["conservativeGrowthBytes"], 5*GIB)

    def test_over_cap_rejected(self):
        with self.assertRaisesRegex(ValueError, "above 5 GiB"):
            accounting(37*GIB-1, 42*GIB, GIB, GIB)
    ...
    def test_scoped_growth_cannot_be_hidden_by_freeing_disk(self):
        with self.assertRaisesRegex(ValueError, "above 5 GiB"):
            accounting(50*GIB, 40*GIB, 3*GIB, 3*GIB)
